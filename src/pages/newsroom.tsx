...
function ArticleEditorModal({ article, onClose, onSave, darkMode }: any) {
  ...
  const handleSave = async () => {
    ...
    let error;
    if (article?.id) {
      try { await articleAction("update", articleData, article.id); } catch(e: any) { error = { message: e.message }; }
    } else {
      try { await articleAction("insert", { ...articleData, author_slug: "editor", author_name: "رئيس التحرير", status: isScheduled ? "scheduled" : "draft", created_at: new Date().toISOString() }); } catch(e: any) { error = { message: e.message }; }
    }
    ...
    <div className="relative z-10 max-w-[1600px] mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* باقي التابات بنفس الطريقة */}
          <div className={`${darkMode ? 'bg-black border-zinc-800' : 'bg-white border-gray-200'} border rounded-lg p-8 text-center text-zinc-500`}>
            <p>جاري تطوير {activeTab === 'submissions' ? 'لوحة المساهمات' : activeTab === 'comments' ? 'لوحة التعليقات' : activeTab === 'media' ? 'مكتبة الوسائط' : 'الإعدادات'} ...</p>
          </div>
        </div>
